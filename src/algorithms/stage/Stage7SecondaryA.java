...
public class Stage7SecondaryA extends Brain {
  ...
  public void step() {
    ...
    // Exploration and enemy detection
    if (state == EXPLORE) {
      for (IRadarResult o: detectRadar()){
        ...
        // convention : whoAmI:TEAM:INSTRUCTION:enemyX:enemyY:OVER
        }
      }
      myMove();
      return;
  ...
  private void process(String message) {
    ...
    if (!(Integer.parseInt(message.split(":")[1])==whoAmI || Integer.parseInt(message.split(":")[1])==TEAM))
        return; 
    System.out.println("Message received: " + message);
    System.out.println( parts);
    String instruction = message.split(":")[2];

    
    if (Integer.parseInt(instruction)==FALLBACK) {
      state = RETREAT;
    }
  }
